    ...
    keys.forEach(key => {
        ...
        input.addEventListener('change', function () {
            if (key === 'day' && input.value === '') {
                ...
                entry[key] = parseInt(input.value);  // Update local value
                sortSchedules();
            }
        });
    });
...
// =========================================
// 🛡️ Días protegidos (copas continentales, Mundial Clubes, amistosos)
// =========================================

// IDs de competiciones continentales que quieres proteger
const CONTINENTAL_COMP_IDS = [
    223, 224, 226, 232, 227, // UEFA (incl. femenino)
    1003, 1014, 1015,        // CONMEBOL
    3005                     // AFC
];

// Mundial de Clubes / Intercontinental
const GLOBAL_TOURNAMENT_IDS = [970, 999];

// Construye un Set de días del año que NO queremos usar
function buildProtectedDaysSet(options) {
    const {
        year,
        avoidContinentalWindow,
        respectWorldClubCupWindow,
        avoidFriendliesQualifiers
    } = options;

    const protectedDays = new Set();
    if (!data['schedule']) return protectedDays;

    // 1) Ventanas alrededor de copas continentales
    if (avoidContinentalWindow) {
        data['schedule'].forEach(entry => {
            if (!CONTINENTAL_COMP_IDS.includes(entry.id)) return;
            const baseDay = parseInt(entry.day, 10);
            if (!Number.isFinite(baseDay)) return;

            // Proteger +/- 3 días
            for (let delta = -3; delta <= 3; delta++) {
                const d = baseDay + delta;
                if (d > 0 && d <= 366) {
                    protectedDays.add(d);
                }
            }
    ...
    // 2) Ventana global para Mundial de Clubes / Intercontinental (julio/agosto)
    if (respectWorldClubCupWindow) {
        const y = year || new Date().getFullYear();
        const july1  = dayOfYearFromDate(new Date(y, 6, 1));  // 1 Jul
        const aug31  = dayOfYearFromDate(new Date(y, 7, 31)); // 31 Ago

        for (let d = july1; d <= aug31; d++) {
            protectedDays.add(d);
        }
    }

    // 3) Amistosos y clasificatorias detectados por nombre (opcional)
    if (avoidFriendliesQualifiers) {
        data['schedule'].forEach(entry => {
            const comp = data['compobj'].find(c => c.line === entry.id);
            if (!comp) return;
            ...
            const isFriendly =
                name.includes('friendly') || name.includes('amistoso');

            const isQualifier =
                name.includes('qualif') ||
                name.includes('clasif') ||
                name.includes('qualification') ||
                name.includes('qualifier');

            if (!isFriendly && !isQualifier) return;

            const baseDay = parseInt(entry.day, 10);
            if (!Number.isFinite(baseDay)) return;

            // Proteger +/- 3 días alrededor de amistosos / clasificatorias
            for (let delta = -3; delta <= 3; delta++) {
                const d = baseDay + delta;
                if (d > 0 && d <= 366) {
                    protectedDays.add(d);
...
function generateAutoSchedule(compLine, config) {
    ...
    // 🛡️ Días protegidos por copas continentales / CWC / amistosos
    const protectedDaysSet = buildProtectedDaysSet({
        year,
        avoidContinentalWindow: !!avoidContinentalWindow,
        respectWorldClubCupWindow: !!respectWorldClubCupWindow,
        avoidFriendliesQualifiers: !!avoidFriendliesQualifiers
    });

    // Default times we’ll use for generated matches
    const defaultTimes = [1755, 2000];  // general slots (1 Friday-ish / 1 weekend prime)
    ...
    for (let round = 1; round <= rounds; round++) {
        ...
        // Espaciado mínimo entre jornadas (5+ días)
        if (lastRoundDate) {
        ...
        // Últimas 3 jornadas de liga: todas a la misma hora
        const isFinalRunIn =
            (type === 'League') && rounds >= 3 && (round > rounds - 3);
        const timesThisRound = isFinalRunIn ? finalRunTimes : defaultTimes;

        let chosenDate = allowedDates[dateIndex];

        // Ajuste para evitar choques / días protegidos
        let attempts = 0;
        while (attempts < allowedDates.length) {
            const dayNum = dayOfYearFromDate(chosenDate);
            let clashFound = false;

            // 1) Evitar días protegidos (copas continentales / CWC / amistosos)
            if (protectedDaysSet && protectedDaysSet.size > 0 && protectedDaysSet.has(dayNum)) {
                clashFound = true;
            }

            // 2) Evitar choques exactos con otros partidos de la misma nación
            if (!clashFound && avoidClashes) {
                const hasClash = timesThisRound.some(time =>
                    clashSet.has(dayNum + ':' + time)
                );
                if (hasClash) clashFound = true;
            }

            // Si no hay choques ni está en día protegido, usamos esta fecha
            if (!clashFound) break;
        ...
        timesThisRound.forEach(time => {
            ...
            // Record clash so later rounds avoid this slot
            if (avoidClashes) {
                const key = day + ':' + time;
                clashSet.add(key);
            }
...
function getConfederationOfCompetition(compLine) {
    const comp = data['compobj'].find(c => c.line === compLine);
    if (!comp) return null;

    // Buscar el nivel 2 (nación)
    let cursor = comp;
    while (cursor && cursor.level > 2) {
        cursor = data['compobj'].find(c => c.line === cursor.parent);
    }

    if (!cursor || cursor.level !== 2) return null;

    const nationId = cursor.line; // level 2 = nation line
    return typeof CONFED_BY_NATION !== 'undefined'
        ? (CONFED_BY_NATION[nationId] || null)
        : null;
}
...
function openScheduleGeneratorModal(compLine) {
    ...
    //Avoid dates near continental cups
    const continentalLabel = document.createElement('label');
    continentalLabel.style.fontSize = '0.75rem';
    continentalLabel.style.color = '#94a3b8';
    const continentalCheckbox = document.createElement('input');
    continentalCheckbox.type = 'checkbox';
    continentalCheckbox.id = 'sched-continental';
    continentalCheckbox.checked = true; // activado por defecto
    continentalLabel.appendChild(continentalCheckbox);
    continentalLabel.appendChild(
        document.createTextNode(' Avoid dates near continental cups')
    );
    form.appendChild(continentalLabel);
    form.appendChild(document.createElement('span'));

    //Block July / Augst for CWC
    const cwcLabel = document.createElement('label');
    cwcLabel.style.fontSize = '0.75rem';
    cwcLabel.style.color = '#94a3b8';
    const cwcCheckbox = document.createElement('input');
    cwcCheckbox.type = 'checkbox';
    cwcCheckbox.id = 'sched-cwc';
    cwcCheckbox.checked = false;
    cwcLabel.appendChild(cwcCheckbox);
    cwcLabel.appendChild(
        document.createTextNode(' Block July / Augst for CWC')
    );
    form.appendChild(cwcLabel);
    form.appendChild(document.createElement('span'));

    // Avoid Friendlies / Qualifiers
    const fqLabel = document.createElement('label');
    fqLabel.style.fontSize = '0.75rem';
    fqLabel.style.color = '#94a3b8';
    const fqCheckbox = document.createElement('input');
    fqCheckbox.type = 'checkbox';
    fqCheckbox.id = 'sched-fq';
    fqCheckbox.checked = false;
    fqLabel.appendChild(fqCheckbox);
    fqLabel.appendChild(
        document.createTextNode(' Avoid Friendlies / Qualifiers')
    );
    form.appendChild(fqLabel);
    form.appendChild(document.createElement('span'));
    ...
    generateBtn.addEventListener('click', () => {
        const config = {
            ...
            // nuevas opciones
            avoidContinentalWindow: continentalCheckbox.checked,
            respectWorldClubCupWindow: cwcCheckbox.checked,
            avoidFriendliesQualifiers: fqCheckbox.checked
        };
